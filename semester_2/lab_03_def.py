...
def table_consol(i, number, interval, root, f_root, iteration, error, error_value):
    ...
    if (type(interval) == str):
        print(
            "|{:^7}|{:^16}|{:^12}|{:^12}|{:^10}|{:^10}|{:^12}|".format(number, interval, root, f_root, iteration, error,
                                                                       error_value))
    elif (type(root) == str):
        print(
            "|{:^7}|   [{:<4};{:>4}]  |{:^12.6}|{:^12.2}|{:^10}|{:^10}|{:^12}|".format(number, interval[0], interval[1],
                                                                                       root, f_root, iteration, error,
                                                                                       error_value))
    else:
        print("|{:^7}|   [{:<4};{:>4}]  |{:^12.6g}|{:^12.2g}|{:^10}|{:^10}|{:^12}|".format(number, interval[0],
                                                                                           interval[1], root, f_root,
                                                                                           iteration, error,
                                                                                           error_value))
    print("+" + "-" * 7 + '+' + "-" * 16 + "+" + ("-" * 12 + "+") * 2 + ("-" * 10 + "+") * 2 + "-" * 12 + "+")

def set_border(f, a, b):
    if f(a) * f(b) <= 0:
        return True
    return False
def method_hord(self, a, b, eps, n):
    ...
    x0 = a - (f(a) / (f(b) - f(a))) * (b - a)  # тоже самое что и x0 = 0 берем произвольное значение из отрезка [0;2]
    global number_root
    global row
    ...
    if (set_border(f, a, b)):
        ...
        while condition:
            ...
            if (set_borders(f, a, b, x0)):
                x1 = x0 - (f(x0) / (f(b) - f(x0))) * (b - x0)
            else:
                x1 = a - (f(a) / (f(x0) - f(a))) * (x0 - a)
            # xn + 1 = g(xn)
        ...
        if flag == 1:
            ...
            table(self, 0, row,
                  [number_root, interval, '{:.6g}'.format(root), '{:.2g}'.format(f(root)), iteration - 1, error, value],
                  'normal')
            ...
            interval = "[ " + str(a) + " ; " + str(b) + " ]"
            table(self, 0, row,
                  [number_root, interval, '{:.6g}'.format(root), '{:.2g}'.format(f(round(root,3))), iteration - 1, error, value],
                  'normal')
...
def main():
    try:
        ...
        table(window_table, 0, row, ["Номер", "Отрезок", "Корень", "f(корень)", "Итерации", "Ошибка", "Пояснение"],
              'bold')
        table_consol(0, "Номер", "Отрезок", "Корень", "f(корень)", "Итерации", "Ошибка", "Пояснение")
        number_root = 1
        for i in np.arange(left, right - step, step):
            method_hord(window_table, i, i + step, eps, n_max)
...
lbl = Label(window, text="Метод хорд", font=("Arial Bold", 13, "bold"))
